...
import com.qm.qmclass.tencent.observer.TICEventObservable;
import com.qm.qmclass.tencent.observer.TICIMStatusObservable;
import com.qm.qmclass.tencent.observer.TICMessageObservable;
import com.qm.qmclass.tencent.utils.CallbackUtil;
...
/////////////////////////////////////////////////////////////////////////////////
    ...
    public int init(Context context, int appId, int disableModule) {
        ...
        //0、给值
        sdkAppId = appId;
        Log.e("TICCCCC",String.valueOf(sdkAppId));
    ...
    public TRTCCloud getTRTCClound() {
        if (mTrtcCloud == null) {
            Log.e("TICManagerImpl","获取TRTCClound对象为空");
            Toast.makeText(QMSDK.getContext(),"获取TRTCClound失败",Toast.LENGTH_SHORT).show();
        }
    ...
    public TEduBoardController getBoardController() {
        if (mBoard == null) {
            Log.e("TICManagerImpl","获取BoardController对象为空");
            Toast.makeText(QMSDK.getContext(),"获取BoardController失败",Toast.LENGTH_SHORT).show();
        }
        return mBoard;
    }
    ...
    public void joinClassroom(final TICClassroomOption option, final TICCallback callback) {
        ...
        if (option == null || option.getClassId() < 0) {
            ...
            Toast.makeText(QMSDK.getContext(),"加入直播间失败option为空",Toast.LENGTH_SHORT).show();
            ...
            public void onError(int errCode, String errMsg) {
                if (callback != null) {
                    if (errCode == 10013) { //you are already group member.
                        Log.e("加入直播间","errCode10013成功");
                        TICReporter.report(TICReporter.EventId.joinGroup_end);
                        onJoinClassroomSuccessfully(callback);
                    }
                    else {
                        Log.e("加入直播间","失败"+errCode + "|"+errMsg);
                        Toast.makeText(QMSDK.getContext(),"加入直播间失败",Toast.LENGTH_SHORT).show();
    ...
    public void joinGroup(final Integer groupId, final TICCallback callback) {
        final String desc = "board group";
        classroomOption=new TICClassroomOption();
        classroomOption.classId=groupId;
        TIMGroupManager.getInstance().applyJoinGroup(groupId.toString(), desc + groupId, new TIMCallBack() {
            @Override
            public void onSuccess() {
                CallbackUtil.notifySuccess(callback,"Success");
                TXCLog.i(TAG, "TICManager: joinClassroom compatSaas onSuccess ");
            }
            ...
            public void onError(int errCode, String errMsg) {
                ...
                if (callback != null) {
                    if (errCode == 10013) { //you are already group member.
                        ...
                        TXCLog.i(TAG, "TICManager: joinClassroom compatSaas onError:" + errCode + "|" + errMsg);
                        CallbackUtil.notifyError(callback,MODULE_IMSDK,errCode,errMsg);
                    }
                }
            }
        });
    }
    public void quitGroup(final Integer groupId, final TICCallback callback) {
        final String desc = "board group";
        classroomOption=new TICClassroomOption();
        classroomOption.classId=groupId;
        TIMGroupManager.getInstance().quitGroup(groupId.toString(), new TIMCallBack() {
            @Override
            public void onError(int errorCode, String errInfo) {
                TXCLog.e(TAG, "TICManager: quitClassroom compatSaas, err:" + errorCode + " msg:" + errInfo);
                CallbackUtil.notifyError(callback,MODULE_IMSDK,errorCode,errInfo);
    ...
    @Override
    public void initBoardAndTRTC(TICClassroomOption option, TICCallback callback) {
        classroomOption = option;
        ...
        if (mTrtcCloud != null) {
            ...
            trtcParams.streamId="camera_"+dataManager.getUserCode();
    ...
    @Override
    public void uninitBoardAndTRTC(boolean clearBoard) {
...
//            customElem.setData(ext.getBytes());
        }
        message.addElement(customElem);
        sendGroupMessage(message, callBack);
    }
...
/////////////////////////////////////////////////////////////////////////////////
    ...
    class TRTCCloudListenerImpl extends TRTCCloudListener {
        ...
        public void onStopPublishing(int i, String s) {
            TXCLog.i(TAG, "TICManager: TRTC 停止推流"+i+" :" + s);
        ...
        public void onRemoteUserEnterRoom(String userId) {
            mEventListner.onUserEnter(userId);
        }

        @Override
        public void onRemoteUserLeaveRoom(String userId, int reason) {
            mEventListner.onUserExit(userId,reason);
...
//            }
    ...
    static class BoardCallback implements TEduBoardController.TEduBoardCallback {
        ...
        public void onTEBError(int code, String msg) {
            ...
            Log.e("白板回调onTEBError",code+"|"+msg);
            Toast.makeText(QMSDK.getContext(),"白板回调失败"+code+"|"+msg,Toast.LENGTH_SHORT).show();
        }
        ...
        public void onTEBWarning(int code, String msg) {
            ...
            Toast.makeText(QMSDK.getContext(),"白板回调失败"+code+"|"+msg,Toast.LENGTH_SHORT).show();
            Log.e("白板回调onTEBWarning",code+"|"+msg);
        }
//        白板初始化完成回调
        @Override
